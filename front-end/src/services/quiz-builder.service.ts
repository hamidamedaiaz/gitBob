  ...
  getFinalQuizSync(): any {
    ...
    const transformedQuestions = (creationData.questions || []).map(q => ({
      ...
      pairs: (q.pairs || []).map(p => ({
        ...
        sourceImage: {
          file: p.sourceImage?.preview || 'placeholder.jpg', // Backend expects a string, not a File object
          preview: p.sourceImage?.preview || 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==' // 1x1 transparent pixel as placeholder
        },
        targetImage: {
          file: p.targetImage?.preview || 'placeholder.jpg', // Backend expects a string, not a File object
          preview: p.targetImage?.preview || 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==' // 1x1 transparent pixel as placeholder
        }
      }))
    }));
...
loadQuizForEdit(quizId: string): Observable<void> {
  return this.getQuizById(quizId).pipe(
    map(quiz => {
      // Convertir le quiz pour le format d'édition
      const editableQuestions: Question[] = quiz.questions?.map(q => ({
        ...
        pairs: q.pairs?.map(p => ({
          description: p.description || '',
          sourceImage: {
            file: new File([], ''), // Fichier vide pour l'édition
            preview: p.sourceImage?.preview || ''
          },
          targetImage: {
            file: new File([], ''), // Fichier vide pour l'édition  
            preview: p.targetImage?.preview || ''
          }
        })) || []
      })) || [];

      // Mettre à jour les subjects avec les données du quiz
...
/**
 * Prépare les données finales pour la mise à jour (similaire à getFinalQuizSync mais pour l'édition)
 */
getFinalQuizForUpdate(): any {
  const creationData = this.creationDataSubject.value;
  const configData = this.configDataSubject.value;

  console.log('Creation data for update:', creationData);
  console.log('Config data for update:', configData);
  ...
  console.log('Final quiz for update:', quiz);
  return quiz;
}
}
